...
INSTALL_ROOT_RE = re.compile(r"\$HERMES_HOME/(?:\$HERMES_HOME/)?skills/")
...
def iter_skills(base: pathlib.Path):
    ...
    for p in sorted(skills_root.rglob("SKILL.md")):
        ...
        if any(part in SUPPORT_DIRS for part in rel.parts[:-1]):
            continue
...
for sk, depth in iter_skills(root):
    ...
    declared = set(re.findall(r"^\s*- key:\s*(\S+)", fm, re.M))
