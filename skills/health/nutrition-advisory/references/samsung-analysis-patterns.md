...
Companion to `references/wearables-data-reliability.md` and the canonical runbook `health.health_dir/samsung-verified-data.md` (confidence tiers, parse gates, audit residuals). Read the runbook first; ALL parse gates must pass before any advice is drawn from the DB. The `samsung-health-import` skill owns refresh/wiring.
