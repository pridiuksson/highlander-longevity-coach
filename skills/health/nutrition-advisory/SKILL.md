...
4. Body: latest body_composition row (validated against your own known values: <YOUR_WEIGHT_KG> weight / <YOUR_SKELETAL_MUSCLE_KG> skeletal muscle)
...
2. **`health.training_plan`** — **STRATEGY + OPERATIONS.** Training program, nutrition strategy, supplement protocol, food framework (including restaurant menu reference with verified nutrition data), biomarker monitoring schedule, phased implementation plan. The separate food guide was deleted and absorbed into this document.
...
**A separate food-guide doc was DELETED** — its content (Tunacado tiers, eat-before-home architecture, carb periodization, restaurant menus, decision tree) was fully folded into `health.training_plan` §3 + §3.5.
