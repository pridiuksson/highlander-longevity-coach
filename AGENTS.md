...
**Publish boundary.** `skills/` is the published kit. `Profile/`, `scripts/`, `ci/` and the root
docs are scaffolding around it. The repo is intended to become public, so everything here is
written as if it already is.
